def flatten(s):
    ...
    for i in s:
        if type(i) != list:
            ...
            # flattened_list += flatten(i) -> equivalent to "extend"
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    if distance(city_a, make_city('', lat, lon)) > distance(city_b, make_city('', lat, lon)) or distance(city_a, make_city('', lat, lon)) == distance(city_b, make_city('', lat, lon)):
...
def count_palindromes(L):
    ...
    2
    """
    palindromes = [i for i in L if i.lower() == i[::-1].lower()]
