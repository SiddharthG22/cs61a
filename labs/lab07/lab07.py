...
class Person:
    ...
    'Would you please preserve abstraction barriers'
    """
    last_said = 'I squirreled it away before it could catch on fire.'

    def __init__(self, name):
        self.name = name
        self.last_said = Person.last_said
        "*** YOUR CODE HERE ***"
...
class SmartFridge:
    ...
    def use_item(self, item, quantity):
        "*** YOUR CODE HERE ***"
        if self.items[item] <= 0 or self.items[item] - quantity <= 0:
            self.items[item] = self.items[item] - min(self.items[item], quantity)
