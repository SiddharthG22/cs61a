...
class VendingMachine:
        ...
        if self.num_items == 0:
            ...
            if self.balance < self.price:
                ...
                if self.balance > self.price:
                    ...
                    self.balance = 0
                    return f"Here is your {self.name} and ${remaining} change."
                else:
    ...
    def restock(self, amount):
        ...
        return f"Current {self.name} stock: {self.num_items}"
...
class Player:
    ...
    def debate(self, other):
        "*** YOUR CODE HERE ***"
        pop_prob = max(0.1, self.popularity / (self.popularity + other.popularity))
        rand_num = make_test_random()
        if rand_num() < pop_prob:
            self.popularity += 50
        else:
            self.popularity -= 50
