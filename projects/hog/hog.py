...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    remaining, last = opponent_score // 10, opponent_score % 10
    if opponent_score >= 100:
        return 2 * abs(remaining % 10 - last) + 1
    return 2 * abs(remaining - last) + 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal):
        for opponent_score in range(goal):
            another_strat = strategy(score, opponent_score)
            if another_strat != strategy(randint(0, 99), randint(0, 99)):
                return False
    return True
...
def make_averaged(original_function, total_samples=1000):
    ...
    def func(*args):
        ...
        while count < total_samples:
            avg_val += original_function(*args)
            count += 1
        return (avg_val / total_samples)
    return func
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while num_rolls <= 10:
        max = make_averaged(roll_dice, total_samples)(num_rolls, dice)
        if max > highest_avg:
            highest_avg = max
            result = num_rolls
        num_rolls += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    if square_update(0, score, opponent_score, dice=six_sided) >= threshold + score:
        return 0
    return num_rolls
